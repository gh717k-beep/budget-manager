...
export default function DashboardScreen() {
  ...
  const payCycle = getPayCycle(yearMonth, budget.payday);
  ...
  const income = sumTransactionsInRange(transactions, 'INCOME', payCycle.start, payCycle.end);
  ...
  const changeMonth = (amount: number) => {
    ...
    setSelectedDate(getPayCycle(nextMonth, nextBudget.payday).start);
    ...
    <SafeAreaView style={appStyles.screen}>
      <ScrollView contentContainerStyle={appStyles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.summaryRow}>
          <Summary label="월 수입" value={income || budget.totalIncome} color={Palette.blue} />
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.daysRow}>
          {days.map((date) => { const active = date === selectedDate; const dateObject = new Date(`${date}T00:00:00`); const day = dateObject.getDate(); const dayOfWeek = dateObject.getDay(); const weekendColor = dayOfWeek === 0 ? Palette.coral : dayOfWeek === 6 ? Palette.blue : Palette.muted; return <Pressable key={date} onPress={() => setSelectedDate(date)} style={[styles.day, active && styles.activeDay]}><View style={{ flexDirection: 'row', alignItems: 'baseline', gap: 2 }}><Text style={[{ color: Palette.muted, fontSize: 9, fontWeight: '700' }, active && styles.activeText]}>{dateObject.getMonth() + 1}월</Text><Text style={[styles.dayNumber, active && styles.activeText]}>{day}</Text></View><Text style={[styles.dayLabel, { color: weekendColor }, active && styles.activeText]}>{['일','월','화','수','목','금','토'][dayOfWeek]}</Text></Pressable>; })}
        </ScrollView>
      ...
      <Modal visible={modalMode !== null} animationType="slide" transparent onRequestClose={closeModal}><View style={styles.modalBackdrop}><View style={styles.modal}>{modalMode === 'transaction' && editing && <TransactionForm value={editing} onChange={setEditing} onSave={() => { saveTransaction(editing); closeModal(); }} onClose={closeModal} />}{modalMode === 'budget' && <BudgetForm value={budget} onChange={setBudget} onSave={() => { saveBudget(budget); setSelectedDate(getPayCycle(yearMonth, budget.payday).start); closeModal(); }} onClose={closeModal} />}</View></View></Modal>
    </SafeAreaView>
  );
}
...
function Field({ label, value, onChange, keyboardType = 'default' }: { label: string; value: string; onChange: (value: string) => void; keyboardType?: 'default' | 'numeric' }) { return <View style={styles.field}><Text style={styles.fieldLabel}>{label}</Text><TextInput value={value} onChangeText={onChange} keyboardType={keyboardType} style={styles.input} placeholderTextColor={Palette.muted} /></View>; }

function ChoiceRow({ options, value, onChange }: { options: string[]; value: string; onChange: (value: string) => void }) { return <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.choices}>{options.map((option) => <Pressable key={option} onPress={() => onChange(option)} style={[styles.choice, value === option && styles.choiceActive]}><Text style={[styles.choiceText, value === option && styles.choiceTextActive]}>{option}</Text></Pressable>)}</ScrollView>; }
